...
```{r, include=TRUE}
strikes_year = bombing[mfunc_desc_class == "KINETIC", .(
    strike_count = .N
...
p_ton = ggplot(strikes_year) + 
    geom_bar(aes(x = as.factor(msnyear), y = tonnage), stat = "identity")
...
Data of U.S. force's fatality in Vietnam War further proved this. In 1968 `r max(fatal_year$fatal_count) %>% comma` U.S. were reportedly deceased (in the article, this number is 14,000). The number decreased gradually in the after years. 
...
On 31st March 1968 Johnson announced on TV that the United States would "restrict bombing of North Vietnam" and pursue negotiation with Hanoi. This is reflected in the following bombing maps:
...
grid.arrange(p65, p66, ncol = 2)
```
We can clearly see the North is bombed quite heavily throughout 1965 - 1967, as well as the famous Ho Chi Minh Trail.

Bombing intensity in the northen part is lessened in 1968...


```{r, include=TRUE, fig.height=7, fig.width=14, fig.align="center"}
p67 = plot_bomb_points(vnmap, target_coord, 1967, nsample = 5000)
p68 = plot_bomb_points(vnmap, target_coord, 1968, nsample = 5000)
grid.arrange(p67, p68, ncol = 2)
```

and came to a stop in 1969 and 1970. However, we can see the U.S. started expanding bombing attacks to Cambodia.
...
In 1969, Nixon became president with a promise to "somehow" end the war. He ordered more and more U.S. soldiers to withdraw, and tried to push Vietnamization strategy but saw little progress. Facing political pressure at home and the army's dissatifcation in the front, starting from 1970 Nixon sent U.S. troops to the neutral Cambodia, as this had been considered the "sanctuary" of Viet Cong and North Vietnam Army that the U.S. dared not to touch. In 1971, to further support Vietnamization:
...
p71 = plot_bomb_points(vnmap, target_coord, 1971, nsample = 5000)
p71
```
...
bombing[, msnmonth := month(msndate)]

bomb_1972 = bombing[msnyear == 1972 & mfunc_desc_class == "KINETIC", .(
    atk_count = .N
    , bomb_tonnage = sum(weapontypeweight * numweaponsdelivered)
    , b52_use = sum(grepl("B52", aircraft_root))
), by = .(msnmonth, msnmonthname)]

bomb_1972[, msnmonthname := reorder(msnmonthname, msnmonth)]

ggplot(bomb_1972, aes(x = msnmonthname, y = b52_use)) + 
