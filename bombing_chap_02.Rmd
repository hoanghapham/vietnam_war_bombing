...
After Geneva conference in 1954, the U.S. replaces France as a political backup for Ngo Dinh Diem in the South Vietnam. They believed in the "domino effect", which suggested that if Vietnam fell under the influence of communism, all countries around would follow. At first, the U.S. believed South Vietnam's government could be grown into a self-sustained one. However, by the beginning of 1965, that belief was turned into "without further American action, Saigon government could not survive." 

Operation Rolling Thunder was devised as a demonstration of such commitment to Saigon. 

The operation lasted from 1965-03-02 - 1968-11-02 with three main objectives which evolved over time:
...
What is interesting here is that the U.S. tried to "tread on thin ice". They wanted to scare North Vietnam off the South, but they did not dare to go for a full-blown air campaign lest China or Soviet would retaliate with their full might. Therefore, their bombing strategy slowly escalated and changed over time, which would reflect to the data.

# Overview

As shared above, the operation started from beginning of 1965. However, we only have data from October of the same year. During this operation, a total of `r sum(thunder$delivered_tonnage) %>% comma()` ton of weapons were dropped on Vietnam.
...
In 1968, Rolling Thunder reached its final stage of operational evolution. Its purpose transformed from psychological warfare in the higher route packages to that of interdicting the logistics flow from the North to the South.

```{r, fig.height=7, fig.width=7, fig.align="center", include=TRUE}
...
theme_clean = theme(
    ...
    , axis.title.x = element_text(color = "gray50", hjust = 0.01)
    , axis.title.y = element_text(color = "gray50", hjust = 1)
...
time_axis = scale_x_date(breaks = seq(date("1966-01-01"), date("1969-01-01"), by = "6 month"), date_labels = "%Y-%m")
...
The increase in intensity also reflects in the tonnage over area. During the peak bombing time of 1968, each square kilometer received more than three times the amount of weapon in the previous peaks. This was due to the increase in bombing tonnage, and the higher focus of targeted area.
...
month_aggr[, ton_per_km2 := total_tonnage / area]
month_aggr[, highlight := msnym >= "1968-01-01"]

year_aggr = month_aggr[, .(avg_ton_km2_monthly = mean(ton_per_km2)), by = .(msnyear)]

vs66 = year_aggr[msnyear == 1968]$avg_ton_km2_monthly / year_aggr[msnyear == 1966]$avg_ton_km2_monthly 
vs67 = year_aggr[msnyear == 1968]$avg_ton_km2_monthly / year_aggr[msnyear == 1967]$avg_ton_km2_monthly 

ggplot(month_aggr) +
    geom_line(aes(x = msnym, y = ton_per_km2, color = highlight, size = highlight, group = 1), show.legend = F) +
    annotate("text", x = date("1968-05-01"), y = 0.8
             , label = "Average tonnage in 1968 is 3 times\nthe averages in 1966 and 1967"
             , hjust = 1, size = 4.5, color = "gray30") + 
    labs(x = "", y = "", title = "Average tonnage per km2 - monthly figures", subtitle = "Unit: ton") + 
    scale_color_manual(values = hlcol) +
    scale_size_manual(values = hlsize) +
    time_axis +
    theme_clean
...
Most of the sorties are related to Strike and Armed reconnaissance missions. While targets of Strike missions were fixed, Armed recon missions' targets are situational. This type of mission made use of small aircraft formations to patroll highways, railroads, rivers and bombed whatever they deemed necessary. 
...
ggplot(mfunc_aggr[1:10]) + 
    ...
    labs(x = "", y = "", title = "Number of mission by functionality") + 
    scale_y_continuous(labels = comma_format()) + 
...
ggplot(aircraft_mfunc[mfunc_desc %in% c("ARMED RECCE", "STRIKE") & sortie_ranking <= 10]) + 
    ...
    geom_text(data = aircraft_mfunc[mfunc_desc %in% c("ARMED RECCE", "STRIKE") & sortie_count >= 14000], 
...
Attack aircrafts were use more and more, indicating more precise bombing missions.
...
ggplot(aircraft_app_time) + 
    ...
    labs(x = "", y = "", title = "Aircraft types usage over time") + 
...
```
 ...
 Meanwhile, pure fighter aircrafts were used less or were adapted into fighter - bomber roles. By this time, dogfighting (air-to-air combat) were fallen out of favor in the U.S. This is due their belief that missiles were all they needed to shoot down Soviet's heavy bombers, and official dogfight training only returned by 1969 with the TOPGUN program.
...
What is a fighter-bomber? According to Wikipedia:
...
The use of A-1 gradually decreased over time as it was to be replace by the A-6A Intruder and A-4 Skyhawk as part of the general switch to jet aircraft.

```{r}
ggplot(aircraft_use_time_service[aircraft_root %in% c("A1", "A4", "A6") & milservice %in% c("USAF", "USMC", "USN")]) + 
    geom_line(aes(x = msnym, y = sortie_count, col = aircraft_root)) + 
    facet_wrap(~milservice, scales = "free", nrow = 3)
...
Bridge, depot and road were bombed most heavily.
...
ggplot(target_time[tgttype %in% top_targets]) + 
    geom_line(aes(x = msnym, y = total_delivered, color = tgttype), stat = "identity", show.legend = F) + 
...
In short, Rolling Thunder was meant to "send a message" to the North, but it was not successful at what it set out to do.
